...
(function (global){
var Stats, THREE, audio, hm, loader, onLoad, render, resize, start, stats, testbed;
...
resize = function(width, height) {
  ...
  return hm.camera.updateProjectionMatrix();
};
...
loader.load('res/model/female04low.json', function(geom, mats) {
...
},{"./audio":52,"./ham":55,"canvas-testbed":2,"stats-js":48}],2:[function(require,module,exports){
...
},{"./types":50}],50:[function(require,module,exports){
var typeMap = {
    'int': 'i',
    'float': 'f',
    'ivec2': 'i2',
...
var Audio, AudioControls, BEAT_MIN, analyser, audio, audioContext, beatCutOff, beatDecayRate, beatHoldTime, beatTime, binCount, freqByteData, levelBins, levelsCount, levelsData, timeByteData, volSens, waveData;
...
Audio = (function() {
  ...
  Audio.prototype.onBeat = function() {
    return this.beatHit = true;
  };

  Audio.prototype.currentTime = function() {
    return this.controls.audio.currentTime;
  };

  Audio.prototype.updateAudio = function() {
    var i, j, k, l, m, ref, ref1, ref2, sum;
    if (!this.isPlaying()) {
      return;
    }
    this.beatHit = false;
    ...
    for (i = k = 0, ref = binCount; 0 <= ref ? k < ref : k > ref; i = 0 <= ref ? ++k : --k) {
      waveData[i] = ((timeByteData[i] - 128) / 128) * volSens;
    }
    for (i = l = 0, ref1 = levelsCount; 0 <= ref1 ? l < ref1 : l > ref1; i = 0 <= ref1 ? ++l : --l) {
      sum = 0;
      for (j = m = 0, ref2 = levelBins; 0 <= ref2 ? m < ref2 : m > ref2; j = 0 <= ref2 ? ++m : --m) {
        sum += freqByteData[(i * levelBins) + j];
      }
      levelsData[i] = sum / levelBins / 256 * volSens;
    }
    sum = levelsData.reduce((function(a, b) {
      return a + b;
    }), 0);
    this.level = sum / levelsCount;
    if (this.level > beatCutOff && this.level > BEAT_MIN) {
      this.onBeat();
      beatCutOff = this.level * 1.4;
      return beatTime = 0;
    } else {
  ...
  Audio.prototype.isPlaying = function() {
    var audio;
    audio = this.controls.audio;
    return !audio.paused && !audio.ended && audio.currentTime > 0;
  };

  Audio.prototype.load = function(url, callback) {
    var self;
    self = this;
    this.controls = new AudioControls(url);
    this.source = this.context.createMediaElementSource(this.controls.audio);
    return this.source.connect(analyser);
  };

  Audio.prototype.startSound = function() {
...
},{"./audiocontrols":53}],53:[function(require,module,exports){
var AudioControls;

module.exports = AudioControls = (function() {
  ...
  AudioControls.prototype.setSrc = function(url) {
    this.audio.src = url;
    return this.audio.load();
  };

  AudioControls.prototype.play = function() {
    if (!this.audio.src) {
      throw Error('No audio URL provided.');
    }
...
var Choreography, eases, i, idx, len, particles, ref, sceneInfo, scenes, shaders, timeline;
...
for (idx = i = 0, len = timeline.length; i < len; idx = ++i) {
  sceneInfo = timeline[idx];
  sceneInfo.endTime = ((ref = timeline[idx + 1]) != null ? ref.beginTime : void 0) || 50;
...
scenes = {
  "default": {
    init: function() {
      this.model.object.position.z = 0;
      return this.camera.position.y = this.model.center().y + 10;
    },
    update: function(s, time) {
      var c;
      c = s.audio.level * 3;
      particles.material.color.setRGB(c, c, c);
      return s.hm.model.dressMaterial.uniforms.time.value = time;
    }
  },
  intro_0_fade: {
    init: function() {
      this.model.setShader(shaders.stripes);
      this.uniforms = this.model.dressMaterial.uniforms;
      this.uniforms.brightness.value = 0;
      this.model.object.position.z = -60;
    ...
    update: function(time) {
      var fadeDuration;
      this.model.object.position.z = -90 + 45 * time / this.sceneInfo.duration;
      this.camera.lookAt(this.model.center());
      fadeDuration = 5;
      return particles.material.color.multiplyScalar(Math.min(time / fadeDuration, 1));
    }
  },
  intro_1: {
    init: function() {
      this.model.setShader(shaders.stripes);
      this.uniforms = this.model.dressMaterial.uniforms;
      this.uniforms.brightness.value = 1;
      this.model.object.position.z = -45;
      this.camera.position.x = -5;
      this.camera.position.z = 70;
      return this.camera.position.y = this.model.center().y + 10;
    },
    update: function(time) {
      this.model.object.position.z = -45 * (1 - time / this.sceneInfo.duration);
      this.camera.lookAt(this.model.center());
      if (time < 2.5) {
        return this.uniforms.balance.value = Math.max(0, time * (this.audio.level - 0.15)) * eases.cubicInOut(time / 4);
      } else {
        return this.uniforms.balance.value = eases.cubicInOut(Math.min(1, time / 7)) * 0.9;
      }
    }
  },
  verse_1_rotate: {
    init: function() {
      this.model.setShader(shaders.turbulence);
      return this.uniforms = this.model.dressMaterial.uniforms;
    },
    update: function(time) {
      this.camera.position.x = 70 * Math.sin(time);
      this.camera.position.z = 70 * Math.cos(time);
      return this.camera.lookAt(this.model.center());
    }
  }
};

module.exports = Choreography = (function() {
  function Choreography(audio, hm) {
    this.audio = audio;
    this.hm = hm;
    this.time = 0;
    this.sceneGraph = this.hm.scene;
    this.camera = this.hm.camera;
    this.lights = this.hm.lights;
  }

  Choreography.prototype.update = function(time) {
    this.time = time;
    if (!this.scene || this.scene.sceneInfo.endTime <= this.time || this.time < this.scene.sceneInfo.beginTime) {
      this.scene = this.startScene(this.findNextScene());
    }
    if (!this.scene) {
      return;
    }
    scenes["default"].update(this, this.time - this.scene.sceneInfo.beginTime);
    return this.scene.update(this.time - this.scene.sceneInfo.beginTime);
  };

  Choreography.prototype.findNextScene = function() {
    var fakeSceneInfo, j, len1, ref1;
    for (j = 0, len1 = timeline.length; j < len1; j++) {
      sceneInfo = timeline[j];
      if ((sceneInfo.endTime > (ref1 = this.time) && ref1 >= sceneInfo.beginTime)) {
        return sceneInfo;
      }
    }
    fakeSceneInfo = {
      name: this.scene.sceneInfo.name,
      duration: this.scene.sceneInfo.duration,
      beginTime: this.time,
      endTime: this.scene.sceneInfo.endTime + this.scene.sceneInfo.duration
    };
    timeline.push(fakeSceneInfo);
    return fakeSceneInfo;
  };

  Choreography.prototype.startScene = function(sceneInfo) {
    var scene;
    scene = scenes[sceneInfo.name];
    scene.sceneInfo = sceneInfo;
    scene.camera = this.camera;
    scene.sceneGraph = this.sceneGraph;
    scene.audio = this.audio;
    scene.model = this.hm.model;
    scenes["default"].init.call(scene, this);
    scene.init(this);
    console.log('starting', sceneInfo.name, 'at', this.time);
    return scene;
  };
...
},{"./particles":57,"./shaders":58,"eases":32}],55:[function(require,module,exports){
(function (global){
var Choreography, GUI, Ham, Model, THREE, audio, particles;
...
Ham = (function() {
  function Ham() {
    ...
    this.autoRotate = 0;
    this.setupScene();
    this.choreography = new Choreography(audio, this);
  }

  Ham.prototype.setupGUI = function() {
    this.gui = new GUI();
    return this.gui.add(this, 'autoRotate', 0, 2);
  };

  Ham.prototype.setupScene = function() {
    var i, len, light, ref;
    this.scene = new THREE.Scene();
    this.camera = new THREE.PerspectiveCamera(70, 1, 30, 1000);
    this.scene.add(this.camera);
    this.lights = [new THREE.PointLight(0xFFFFFF, 0.1), new THREE.PointLight(0xFFFFFF, 0.1), new THREE.PointLight(0xFFFFFF, 0.0), new THREE.PointLight(0xFFFFFF, 0.0)];
    this.lights[0].position.set(80, 80, -10);
    this.lights[1].position.set(-80, 80, 10);
    this.lights[2].position.set(80, 70, 80);
    this.lights[3].position.set(-80, 70, -80);
    ref = this.lights;
  ...
  Ham.prototype.setupRenderer = function(runner, canvas) {
    this.runner = runner;
    this.canvas = canvas;
    ...
    return this.renderer.setClearColor(0x0, 1);
  };

  Ham.prototype.loadModel = function(geom, mats) {
    this.model = new Model(geom, mats);
    return this.scene.add(this.model.object);
  };

  Ham.prototype.tick = function(dt) {
    this.dt = Math.max(audio.currentTime() - this.time, 0);
    this.time = audio.currentTime();
    audio.updateAudio();
    this.choreography.update(this.time);
    return this.model.updateAnimation(this.dt, this.time);
  };

  Ham.prototype.render = function() {
...
},{"./audio":52,"./choreography":54,"./model":56,"./particles":57,"dat-gui":11}],56:[function(require,module,exports){
(function (global){
var Model, THREE;
...
module.exports = Model = (function() {
  function Model(geom, mats) {
    var i, idx, len, mat;
    for (idx = i = 0, len = mats.length; i < len; idx = ++i) {
      mat = mats[idx];
      if (mat.name === 'DRESS.001') {
        this.dressMaterial = mat;
        this.dressMaterialIndex = idx;
      }
    ...
    this.object = new THREE.Mesh(geom, new THREE.MeshFaceMaterial(mats));
    this.object.scale.multiplyScalar(50);
    this.object.position.y = -50;
    this.setupAnimation();
  }

  Model.prototype.center = function() {
    return new THREE.Vector3().addVectors(this.object.position, new THREE.Vector3(0, 55, 0));
  };

  Model.prototype.setShader = function(shader) {
    var mat;
    this.dressMaterial.dispose();
    mat = new THREE.ShaderMaterial(shader);
    this.dressMaterial = mat;
    this.object.material.materials[this.dressMaterialIndex] = mat;
    return mat;
  };

  Model.prototype.setupAnimation = function() {
    this.animation = new THREE.MorphAnimation(this.object);
    this.animation.duration = (1000 * 60 / 100) * 2;
    this.animation.frames -= 0.00000001;
    return this.animation.play();
  };

  Model.prototype.updateAnimation = function(dt, time) {
    return this.animation.update(dt * 1000);
  };
...
var i, p, pGeometry, pMaterial, pX, pY, pZ, particles;
...
var k, setupShader, shaderCreators, shaders, v;
...
setupShader = function(shaderCreator) {
  var shader;
...
},{"../shaders/checker":59,"../shaders/curl":60,"../shaders/stripes":61,"../shaders/turbulence":62}],59:[function(require,module,exports){
(function (global){
var glslify = require("glslify");
var THREE, createShader, source, xtend;
THREE = (typeof window !== "undefined" ? window.THREE : typeof global !== "undefined" ? global.THREE : null);
xtend = require("xtend");
...
},{"glslify":46,"glslify/simple-adapter.js":47,"three-glslify":49,"xtend":51}]},{},[1]);
